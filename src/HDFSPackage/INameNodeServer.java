...
public class INameNodeServer implements INameNode {
	...
	public byte[] openFile(byte[] input) { 
		...
		if (openFileRequest.forRead == true) { 
			if (nameToBlocks.containsKey(openFileRequest.fileName)
					&& !openForWrite.contains(openFileRequest.fileName)) { 
	...
	public byte[] assignBlock(byte[] input) { 
		...
		System.out.println(
				assignBlockResponse.newBlock.blockNumber + ":" + assignBlockResponse.newBlock.locations.size());
